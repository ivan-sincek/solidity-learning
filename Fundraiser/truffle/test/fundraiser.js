...
contract("Fundraiser", accounts => {
	...
	const hacker         = accounts[3]; // malicious actor
	const philanthropist = accounts[4]; // initial philanthropist (donor)
	...
	const amountZero = web3.utils.toWei("0");
	...
	describe("Initializations", async () => {
		...
		it("Get Custodian", async () => {
			const custodianCurrent = await fundraiser.custodian();
		...
		it("Get Beneficiary", async () => {
			const beneficiaryCurrent = await fundraiser.beneficiary();
		...
		it("Get Information", async () => {
			const informationCurrent = await fundraiser.information();
			assert.equal(informationCurrent.beneficiary, beneficiary, "Current beneficiary does not match the initial beneficiary.");
			assert.equal(informationCurrent.name, name, "Current name does not match the initial name.");
			assert.equal(informationCurrent.description, description, "Current description does not match the initial description.");
			assert.equal(informationCurrent.website, website, "Current website does not match the initial website.");
			assert.equal(informationCurrent.thumbnail, thumbnail, "Current thumbnail does not match the initial thumbnail.");
		});
		...
		it("Set Zero Address as Custodian", async () => {
			try {
				...
				assert.fail("Zero address is allowed.");
			} catch (err) {
				const exceptionCurrent = err.reason;
				const exceptionExpected = "Zero address is allowed.";
				assert.equal(exceptionCurrent, exceptionExpected, "Current exception does not match the expected exception.");
			}
		});
