...
export default class Donate extends Component {
	...
	async donate(event) {
		...
		if (this.state.globalError) {
			this.setState({ globalMessage: "Web3.0 not initialized" });
		} else {
			this.setState({ globalMessage: "" });
			let error = false;
			if (this.state.amount.length < 1) {
				error = true;
				this.setState({ amountMessage: "Amount is required" });
			} else if (this.state.amount <= 0) {
				error = true;
				this.setState({ amountMessage: "Amount must be greater than zero" });
			} else {
				this.setState({ amountMessage: "" });
			}
			if (this.state.gasPrice.length > 0 && this.state.gasPrice <= 0) {
				error = true;
				this.setState({ gasPriceMessage: "Gas price must be greater than zero" });
			} else {
				this.setState({ gasPriceMessage: "" });
			}
			if (!error) {
				let transaction = {
				...
				this.setState({
					globalMessage: "Thank you for your donation"
				});
			}
		}
	}
			...
			<div className="donate">
				<div className="layout">
					<header>
						...
						<p>Raised (ETH): <span>{this.state.raised}</span></p>
						<p>Donated (ETH): <span>{this.state.donated}</span></p>
					</header>
				...
				<div className="table">
					<table>
						...
						<tbody>
							{this.state.history && this.state.history.map((transaction, index) => {
								return (
									<tr key={index}>
