...
contract("Metamorphic", (accounts) => {
	...
	describe("Metamorphic Malware", async () => {
		...
		it("Validate Same Addresses", async () => {
			...
			const addressProxyFirst = txFirst.logs[2].args["_addressProxy"];
			const addressTargetFirst = txFirst.logs[5].args["_addressTarget"];
			await contract.destroy();
			// ----------
			const txSecond = await contract.create(salt);
			const addressProxySecond = txSecond.logs[2].args["_addressProxy"];
			const addressTargetSecond = txSecond.logs[5].args["_addressTarget"];
			await contract.destroy();
			// ----------
		...
		it("Metamorph", async () => {
			...
			const addressTarget = tx.logs[5].args["_addressTarget"];
